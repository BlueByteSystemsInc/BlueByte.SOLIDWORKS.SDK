...
namespace BlueByte.SOLIDWORKS.SDK.UI
{
    public static class WindowsHelper
    {
        ...
        public static DialogResult ShowMessageBox(string message, bool topMost = true
           , string title = null, MessageBoxButtons buttons = MessageBoxButtons.OK)
        {
            if (topMost)
            {
                ...
                return (DialogResult)MessageBoxUser32(0, message, title == null ? string.Empty : title, MB_TOPMOST);
            }
            else
            {
                return MessageBox.Show(message, title == null ? string.Empty : title, buttons);
            }
        }
    }
}
